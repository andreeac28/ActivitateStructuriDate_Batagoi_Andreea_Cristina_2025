...
struct Masina initalizare(int id, int nrLocuri, const char* marca, float capacitateC, char normaPoluare)
{
	struct Masina m;
	m.id = id;
	m.nrLocuri = nrLocuri;
...
struct Masina* (copiazaPrimeleMasini(struct Masina* vector, int nrElemente, int nrElementeCopiate))
{
	...
	{
		vectorNou[i] = initalizare(vector[i].id, vector[i].nrLocuri, vector[i].marca, vector[i].capacitateC, vector[i].normaPoluare);
	}
	return vectorNou;
}
...
struct Masina getPrimaMasinaDupaMarca(struct Masina* vector, int nrElemente, const char* conditie)
{
	...
	return initalizare(1, 0, NULL, 0, '0');
}
...
int main()
{
	...
	vector[0] = initalizare(1, 3, "Dacia", 40, '5');
	vector[1] = initializare(2, 5, "Honda", 30, '6');
	vector[2] = initalizare(3, 4, "Dacia", 50, '3');
